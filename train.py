...
import os
from models import SimpleConv, ResNet, RnnNet, Cldnn
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default='SimpleConv')
    ...
    dm = RML2018DataModule('/work/ds2/data/k.witham/RML2018.01/GOLD_XYZ_OSC.0001_1024.hdf5', '/scratch/k.witham', args.bs, use_hard = True)

    match args.model:
        case 'SimpleConv':
            model = SimpleConv(classes=dm.classes)
        case 'ResNet':
            model = ResNet(classes=dm.classes)
        case 'RnnNet':
            model = RnnNet(classes=dm.classes)
        case 'Cldnn':
            model = Cldnn(classes=dm.classes)
